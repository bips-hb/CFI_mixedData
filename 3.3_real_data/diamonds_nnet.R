##################
# same code as diamonds.R, but with neural network as a learner 
# same data set (train/test) is ensured via set.seed(1)
##################
...
# read data
df = readARFF("./knockoffs/real_data/diamonds_data/dataset.arff")
df = df[which(abs(df$x - df$y) < 0.02),]
levels(df$cut) = letters[1:length(levels(df$cut))]
# rename variables
...
# Boruta 
boruta = Boruta(y  ~ . , data = test)
boruta$finalDecision  

# preprare for model training
my_learner <- lrn(paste0("regr.", learner), predict_type = "response")
if (learner == "nnet"){my_learner$param_set$values = list(size = 100, decay = 0.15, maxit=3000,MaxNWts = 90000)}
...
# get R squared
print("This is R squared")
rsq = fit$predict_newdata(newdata = test)$score(measures = msr("regr.rsq"))
print(rsq) # R squared is 0.9348747 
...
# SAGE
source("./knockoffs/sim_FI_comparison/SAGE_R_efficient.R")
# rename target to "y"
...
write.csv(apply(res,2,as.character), file = paste0("./knockoffs/real_data/res_diamonds_", learner, ".csv"))
