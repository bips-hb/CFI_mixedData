...
binary_var_app <- list(c("all Gaussian"),c("X1"), c("X3"))
...
algo_design_app <- list(
  validation = expand.grid(learner = learners_app, dummy_enc=dum)
)
...
write.csv(apply(res,2,as.character), file = "./validate_models/validate_for_sim_batchtools.csv")
