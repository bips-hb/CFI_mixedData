##################################
# THIS IS THE EXACT SAME CODE TO GENERATE DATA AS IN problems.R -> GENERATED DATA SETS ARE IDENTICAL!
# THE ONLY DIFFERENCE IS THAT FOR CLASSIFICATION TASKS THE CLASS PROBABILITIES ARE RETURNED AS WELL
# THIS INFORMATION IS IMPORTANT FOR MODEL VALIDATION 
##################################
library(dplyr)
library(fastDummies)
...
DAG_cat = function(n, effect_size,outcome, cat_variables, num_levels){
  ...
  if(outcome == "regr"){ 
    y = x3.relevant_internal + x4.relevant_internal + rnorm(n, mean = 0, sd = 1)
    ret = data.frame(x1.irrelevant,x2.irrelevant,x3.relevant, x4.relevant, y)}
  if(outcome == "classif") {
    prob = plogis(x3.relevant_internal - x4.relevant_internal)
    y = rbinom(n = n, 1, prob = prob)
    #  print(prob)
    ret = data.frame(x1.irrelevant,x2.irrelevant,x3.relevant, x4.relevant, y, prob) ## ADD PROB TO RETURNED DATA FRAME 
  }
  return(ret)
}
